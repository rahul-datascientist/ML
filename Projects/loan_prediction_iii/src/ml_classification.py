...
from sklearn.model_selection import KFold  ## Creating cross validation sets
...
def runET(train_X, train_y, test_X, test_y=None, test_X2=None, rounds=100, depth=20,
    ...
    if test_y is not None:
        ...
        print("Train and Test loss : ", train_loss, test_loss)
        accuracies = cross_val_score(estimator=model, X=train_X, y=train_y.values.ravel(), cv=10)
        print("Train Cross Validation mean : ", accuracies.mean())
        print("Train Cross Validation Std : ", accuracies.std())
        accuracies = cross_val_score(estimator=model, X=test_X, y=test_y.values.ravel(), cv=10)
        print("Test Cross Validation mean : ", accuracies.mean())
        print("Test Cross Validation Std : ", accuracies.std())
        return test_preds, test_loss, test_preds2, model
...
def runSVC(train_X, train_y, test_X, test_y=None, test_X2=None, C=1.0,
    ...
    print("Test Cross Validation Std : ", accuracies.std())
    if grid_search == 'Y':
        parameters = {'C': [1.0, 10.0, 20.0, 50.0, 100.0, 1000.0], 'kernel': ['linear', 'poly', 'rbf', 'sigmoid'],
                      'gamma': [0.1, .001, 1.0, 10.0, 20.0, 50.0, 100.0, 1000.0]}
